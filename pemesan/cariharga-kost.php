...
<div class="wrapper row3">
    <div class="rounded">
         <form action="#" onsubmit="setDirections(this.dari.value, this.sampai.value, this.value); return false">
...
$sql = "select * from rumah_kost harga_terendah between  '$dari' and '$sampai'";
...
while ($data = mysql_fetch_array($query)) {
    ?>
                ...
                marker.setIcon({ url: "icon.png", scaledSize: new google.maps.Size(30, 24) , anchor: new google.maps.Point(15, 12)});
                markers.push(marker);
        ...
        $('#cari').change(function(){
            ...
            <body onLoad="initialize()">
                ...
                <select id="cari" name="dari">
                    ...
                    ?> </select>
                           <select id="cari" name="sampai">
                    <?php
                    $sql = "select * from rumah_kost order by Nama_kost";
                    $query = mysql_query($sql) or die(mysql_error());
                    $n = 0;
                    while ($data = mysql_fetch_array($query)) {
                        ?>
                        <option value="<?php echo $n; ?>"><?php echo $data['Nama_kost']; ?></option>
                        <?php
                        $n++;
                    }
                    ?>
                ...
                <td colspan="4"><input type="submit" name="button" id="button" value="cari"></td>
            </body>
        </main></div></div>